...
#
# 示例:
#   bash scripts/cut_county.sh 天台县 /Users/tianli/Dev/Work/resources/gis/geojson/台州市/天台县
...
示例:
  $0 天台县 /Users/tianli/Dev/Work/resources/gis/geojson/台州市/天台县
...
EXPECTED=(县界 河流 水库点 大中型水库 小水电)
ALL_OK=1
for name in "${EXPECTED[@]}"; do
    f="${OUTPUT_DIR}/${name}.geojson"
    if [ -f "$f" ]; then
        size=$(stat -f%z "$f" 2>/dev/null || stat -c%s "$f" 2>/dev/null)
        # 用 python 读 feature 数（跨平台稳）
